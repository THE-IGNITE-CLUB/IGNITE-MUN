...
export default function SuperAdminLogin() {
  ...
  const submit = async (e) => {
    ...
    setLoading(true)
    try {
      const data = await login(form.user_id.trim(), form.password)
      if (data.role === 'super_admin') {
        toast.success('Welcome, Super Admin!')
        navigate('/admin/super')
      } else if (data.role === 'eb' || data.role === 'admin' || data.role === 'oc') {
  ...
  return (
    <PageWrapper>
      ...
      <main className="relative flex-grow min-h-screen flex items-center justify-center p-[16px] md:p-[48px] overflow-hidden pt-28">
        {/* Background — identical to Login.jsx */}
        ...
        {/* Card — same shape, shadow and width as Login.jsx */}
        <div className="relative z-20 w-full max-w-md bg-surface-container-lowest rounded-xl border border-outline/10 shadow-[0_24px_48px_-12px_rgba(15,23,42,0.2)] overflow-hidden">
          ...
          {/* Header — same structure as Login.jsx but with admin icon */}
          <div className="bg-surface-container-low p-8 text-center border-b border-outline/10 relative overflow-hidden">
            ...
            {/* Logo circle matching Login.jsx */}
            ...
            <h1 className="font-headline-lg text-headline-lg text-primary mb-2 relative z-10">Admin Portal</h1>
            <p className="font-body-md text-body-md text-on-surface-variant relative z-10">IGNITE MUN 2026 · Secretariat Access</p>
          </div>

          {/* Form — identical layout to Login.jsx */}
          <div className="p-8">
            <form onSubmit={submit} className="space-y-6">

              <div className="space-y-2">
                <label className="block font-label-md text-label-md text-on-surface" htmlFor="user_id">Admin Username</label>
              ...
              <div className="mt-4 pt-4 border-t border-outline/10 text-center flex items-center justify-center gap-2">
                ...
                <span className="font-label-sm text-label-sm text-on-surface-variant">Encrypted · Restricted Access</span>
              </div>
            </form>

            <p className="text-center mt-4 text-sm text-on-surface-variant font-body-md">
              Delegate?&nbsp;
              <Link to="/login" className="text-secondary hover:underline font-semibold">Login here</Link>
            </p>
          </div>
        </div>
      </main>
